...
P0973r0 paper identified a number of issues that make coroutines in their current form sub-optimal for exception-less error propagation. We fully agree with some of the issues (for example, awkwardness of `co_return` and `co_await` keywords in that scenario and inability to mark coroutines as `constexpr`), but we categorically, absolutely, emphatically, vociferously object to mistaken notion that coroutines violate zero-overhead principle.
Before diving into details of why P0973r0 is mistaken on this issue, let's go through the areas of agreement (or mostly agreement).
...
Now let's proceed to most controversial and most spectacularly wrong claim of P0973r0 with which we disagree violently and emphatically!
