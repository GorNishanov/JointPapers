...
# A Response to "D0973r0: Coroutines TS Use Cases and Design Issues"
...
* asynchronous streams, which can be thought as an asynchronous version of a generator where both `co_await` and `co_yield` can be used. 
...
D0973r0 paper identified a number of issues that make coroutines in their current form sub-optimal for exception-less error propagation. I fully agree with some of the issues (for example, awkwardness of `co_return` and `co_await` keywords in that scenario and inability to mark coroutines as `contexpr`), but I categorically, absolutely, emphatically, vociferously object to mistaken notion that coroutines violate zero-overhead principle.
Before diving into details of why D0973r0 is mistaken on this issue, let's go through the areas of agreement (or mostly agreement).

<!--This fundamental mistake significantly undermines the strength of arguments made and conclusions reached in D0973r0.
-->

## Const reference parameters are dangerous <a ref="#references"></a>

We agree with D0973 point here and would go even further. Any reference or raw pointer is dangerous if that reference/pointer survives and used after the lifetime of an object has ended.
...
Though standard coroutine types are unlikely to be that drastic, custom coroutine types used in a particular codebase by a particular company can chose to ban reference and pointer arguments to a coroutine with an escape hatch with some version of `std::ref` wrapper, for example. It only requires a little bit of template meta-programming and a `static_assert` when defining a coroutine type.
...
## co_await nests awkwardly <a ref="#await-composition"></a>
...
Coming back to the issue of using parentheses when mixing postfix and prefix operators raised in D0973r0:
...
Now let's proceed to most controversial and most spectacularly wrong claim of D0973r0 with which we disagree violently and emphatically!
...
```
The requirements for this optimization to occur is that your coroutine type has to have RAII semantics, some members of the coroutine type (i.e. `generator` or `task`) should be available for inlining and the lifetime of the coroutine does not escape its caller. Here are the examples where there **will be** an allocation of memory (and coroutine designer can control what allocator is used if desired).
...
Let me briefly restate authors points in the condensed form (please see the original paper [D0973r0] for full context):

* not clear it is feasible in general, clang for example only do it when the coroutine is inlined
...
> * not clear it is feasible in general, clang for example only do it when inlined
...
Coroutines absolutely do not have to be inlined for heap allocation elision optimization to be in effect. Moreover, Richard Smith, Chandler Carruth and Gor Nishanov, on one rainy evening of 2014, sketched out how this optimization would work across hard ABI boundaries where peeking at the body of the coroutine is impossible.

With respect to feasibility of this optimization in general, if there is a **reasonable doubt** that this optimization is not feasible for the designed use cases, we need to absolutely stop work on the Coroutines TS and look for alternatives!
...
We are in full agreement with authors of D0973 on this subject. **Moreover**, efficient use of coroutines with `expected<T,E>` **absolutely, does not rely on the heap elision optimization at all!** We only rely on Coroutine TS blessing not to have an allocation if not needed.
...
We thanks the authors of D0973r0 for taking time to try out coroutines and write a concerns paper. We are in agreement with some of the concerns, believe that some are not relevant to Coroutines TS as a solution is available via library bindings, and other concerns can be addressed with small non-breaking improvements, if desired, to improve support for non-exceptional error propagation via `expected<T,E>`.

D0973r0 conclusion that coroutines are not well suited for generator and `expected<T,E>` use cases is based primarily on the alleged unfeasibilty and untrustworsiness of elidable implicit allocation of the coroutine frame and inability to control allocations. The demonstrated unfamiliarity with coroutine allocation customization techniques, optimizations used to implement coroutines in Clang compiler and obviously untrue statements made in "Concerns about implicit elidable coroutine frame allocation" section of D0973r0 undermine the credibility of the paper's conclusion.
...
* D0973r0: "Coroutines TS Use Cases and Design Issues" shared on reflector
